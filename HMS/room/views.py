...
def rooms(request):
    ...
    firstDayStr = None
    lastDateStr = None
    # 定義了一個名為chech_availability的函數，用於檢查房間在給定日期範圍內的可用性
    ...
    # 如果請求中包含了"dateFilter"，則從請求中獲取起始日期和結束日期，並調用chech_availability函數篩選可用的房間
...
def current_room_services(request):
    ...
    # 如果請求中包含了"foodReq"，則創建一個新的房間服務請求對象（RoomServices）並將其保存到數據庫中，同時設置相關的屬性（例如curBooking、price、room和servicesType）
    if request.method == "POST":
...
def booking_success(request):
    ...
    return render(request, path + "booking_success.html", context)
